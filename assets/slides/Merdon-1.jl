...
# ╔═╡ 82fa48ac-282c-11f1-b1e8-dd3476c48852
begin
	using ExtendableFEM
...
# ╔═╡ 3affe620-59a4-48f7-9bc7-ab0c85beccf5
md"""
# WIAS-PDELib: A Julia PDE solver ecosystem - part 2
...
# ╔═╡ 6677f063-54c3-417e-93f2-84d712b3ef88
md"""
In this example we want to solve a transport problem with a convection field that is obtained from a FEM disretization of the Stokes problem with axisymmetric data (3D domain by revolution of a 2D domain with cylindrical coordinates, invariant with respect to angle)
"""

# ╔═╡ 3c633fdd-1981-4788-a261-e7e0ea9cea7c
md"""
(See [WIAS-Preprint 3264](https://www.wias-berlin.de/publications/wias-publ/run.jsp?template=abstract&type=Preprint&year=2026&number=3264) for details)
"""

# ╔═╡ f0bca720-5ea9-43a3-a21d-77e1e9ff6a5c
md"""
### Step 1 : Solving the axisymetric Stokes problem
"""

# ╔═╡ e77e62ea-cebd-4a2f-b0ee-63fddbddd113
md"""
This example solves the 2D axisymmetric formulation of the Navier--Stokes problem that seeks a velocity ``\mathbf{u} = (u_z, u_r)`` and pressure ``p``
such that
```math
\begin{aligned}
a(u,v) + b(p,v) & = (f,v) \\
...
= \int_{\Omega} q \, \mathrm{div}(rv) dr dz
\end{aligned}
```
(for more details see [Example260 in the ExtendableFEM documentation](https://wias-pdelib.github.io/ExtendableFEM.jl/stable/module_examples/Example260_AxisymmetricNavierStokesProblem))
"""

# ╔═╡ c742017d-7d2a-41ea-a170-c5002acb8e2e
md"""
The grid is an L-shaped domain...
"""

# ╔═╡ 938720a0-c926-48c1-8c38-5d880990250a
flowgrid = uniform_refine(grid_lshape(Triangle2D; shift = [1,0]), 3)

# ╔═╡ 2ed8f1da-2e3e-4e66-8b01-45fafcae536b
gridplot(flowgrid; Plotter = CairoMakie, linewidth = 1, size = (650,400))

# ╔═╡ 43318b9a-5c6b-4b92-b0af-bd5f8d9d9636
md"""
The **ProblemDescription** involves the two unknowns velocity and pressure
"""

# ╔═╡ 88882e31-c213-4c3d-9cff-bd04bad737ab
begin
    u = Unknown("u"; name = "velocity")
    p = Unknown("p"; name = "pressure")
end

# ╔═╡ ba4c6d16-fcf7-4993-9a01-85d5a7f97d18
md"""
The next function computes the velocity by a FEM (P2-bubble x P1-disc)
"""

# ╔═╡ fad58cbf-b503-4146-b4ee-c5b206f9960d
function compute_velocity(flowgrid, μ = 1.0e-3)
	...
	## axisymmetric Stokes operator
	function kernel_stokes!(result, u_ops, qpinfo)
	    μ = qpinfo.params[1] # qpinfo knows params, space coordinates, normals etc.
		r = qpinfo.x[1] # first component of space coordinate is radius
    ...
    assign_operator!(
        Problem,
        BilinearOperator(
            kernel_stokes!, 			# kernel function (above)
			[id(u), grad(u), id(p)];    # operators for ansatz and test function
            bonus_quadorder = 2,
            params = [μ]                # extra parameters, comes in as qpinfo.params
        )
    )
	...
	assign_operator!(
		Problem, InterpolateBoundaryData(u, inflow_cylindrical; regions = [7])
	)
	assign_operator!(Problem, HomogeneousBoundaryData(u; regions = [1,3,4,8]))
	
    # define finite element spaces
    FE_v, FE_p = H1P2B{2, 2}, L2P1{1}
    FES = [FESpace{FE_v}(flowgrid), FESpace{FE_p}(flowgrid)]

	# solve (with unicode plots)
    sol = solve(Problem, FES; plot = true)

    return sol
end

# ╔═╡ 46c78465-f176-44b3-b946-4c16bee45606
sol = compute_velocity(flowgrid, true)

# ╔═╡ b08f7d48-1bd3-4165-b641-dfacbab821b7
reveal(ExtendableFEM.plot([id(u), id(p)], sol; Plotter = CairoMakie, width = 650))

# ╔═╡ 16e033aa-9db8-4fb6-8f1d-3f1a8a7201a6
md"""
### Step 2 : A divergence-free postprocessing
"""

# ╔═╡ 14c2a154-22e3-4e4c-8b87-7e6a83fc3f51
md"""
Since the discrete FEM solution does not satisfy $\mathrm{div}(ru) = 0$,
we will compute a postprocessing into a $\mathrm{H}(div)$-conforming space,
that is really divergence-free. This property will become important for coupled
transport processes.
"""

# ╔═╡ 82e317cb-a369-4ee3-99c9-6a245947474b
begin
	# divergence of original solution
	function multiply_r(result, input, qpinfo)
    	result .= input * qpinfo.x[1]
	end
	DivIntegratorR = L2NormIntegrator([div(u)]; kernel = multiply_r, quadorder = 2, resultdim = 1)
	div_v = sqrt(sum(evaluate(DivIntegratorR, sol)))
	@info "||div(ru)||_2 = $(div_v)"
	
    # ensure divergence free solution by projecting onto reconstruction spaces
    FES_reconst = FESpace{HDIVBDM2{2}}(flowgrid)
    solR = FEVector(FES_reconst)
	lazy_interpolate!(
		solR[1], sol, [id(u)];
		postprocess = multiply_r,
		bonus_quadorder = 2
	)

	# divergence of reconstruction
	DivIntegratorR = L2NormIntegrator([div(1)]; quadorder = 2, resultdim = 1)
	div_v = sqrt(sum(evaluate(DivIntegratorR, solR)))
	@info "||div(R(ru))||_2 = $(div_v)"
end

# ╔═╡ 9a04e46a-9aba-434a-8dfe-8258040d75dd
reveal(ExtendableFEM.plot([div(1)], solR; Plotter = CairoMakie, width = 650))

# ╔═╡ 9a2d2ffb-62ba-4ee9-9091-ed62df5399a7
md"""
### Step 3 : Conservative FVM transport

Now we want to use the discrete velocity field in a convection diffusion transport equation of some species $c$, namely
```math
\begin{aligned}
- \nabla \cdot (D \nabla c - \mathbf{u} c) = 0
\end{aligned}
```
with inlet concentration $c_\text{in} = 1$ at the bottom boundary of the domain. If no further reactions take place, the min/max principle holds and we expect that
the solution is $c \equiv 1$ everywhere. However, this only is the case when $\mathbf{u}$ (or $r \mathbf{u}$ in the axisymmetric case) is divergence-free.
"""

# ╔═╡ 7f00d112-d984-4624-bd63-a00d49b5711b
begin
	# use a different grid of the same domain
	chemgrid = uniform_refine(grid_lshape(Triangle2D; shift = [1,0]), 4)
	
	# the finite volume machinery relies upon the CoordinateSystem for
	# selecting the correct geometrical factors for the Voronoi cell contributions
	chemgrid[CoordinateSystem] = Cylindrical2D
	chemgrid
end

# ╔═╡ 452b57d7-e935-4468-8697-b0c13eae7dc7
md"""
Use the div-free reconstruction: $(@bind use_reconstruction Switch(default=false))
"""

# ╔═╡ afdd214f-ae66-4ffc-96a5-8913505ba9b0
begin
	## Problem data structure
	mutable struct ProblemData
	    D::Float64      # diffusion coefficient
	    cin::Float64    # inlet concentration
	...
	# compute edge and bface velocities (fluxes over control volume edges)
	# = integrates velocity field over segments of flowgrid
	if use_reconstruction
		evelo = edgevelocities(chemgrid, solR[1]; reconst = true)
		bfvelo = bfacevelocities(chemgrid, solR[1]; reconst = true)
	else
		evelo = edgevelocities(chemgrid, sol[u]; reconst = false)
		bfvelo = bfacevelocities(chemgrid, sol[u]; reconst = false)
	end
		
	data.evelo = evelo
	data.bfvelo = bfvelo

	# convection-diffusion flux in transport equation
	# two-point expoential flux (Scharfetter-Gummel)
	# -> preserves min/max principles when div(rv) = 0
	...
	# boundary conditions for FVM system
	function bconditions!(f, u, node, data)
		...
		# rest has inert wall conditions -> do-nothing
	    return nothing
	end

	# define FVM problem
	physics = VoronoiFVM.Physics(; flux = flux!, breaction = bconditions!, data)
	sys = VoronoiFVM.System(chemgrid, physics)
	enable_species!(sys, 1, [1])

	# solve the FVM problem
	sol_vfm = solve(sys; inival = 0.0)

	
	flowgrid[CoordinateSystem] = Cartesian2D # ensure this in case of reevaluation
end

# ╔═╡ 1e7c4069-5728-48dd-90fe-c956938885eb
begin
	# finite volume divergences = sum of rv*n along control volume boundaries
	fvm_divs = VoronoiFVM.calc_divergences(sys, evelo, bfvelo)
	@info "||div(v)||_∞ = $(norm(fvm_divs, Inf))"

	# check min/max principle
	minmax = extrema(sol_vfm)
	@info "Minimal/maximal values of concentration: $(minmax)"
end

# ╔═╡ 03df2707-4d15-43c4-a2ed-4808a2b29885
begin
	vis = GridVisualizer(; Plotter = CairoMakie)
	scalarplot!(vis[1, 1], chemgrid, sol_vfm[1, :]; show = true, flimits = (minmax[1] - 1.0e-6, minmax[2] + 1.0e-6))
end
...
manifest_format = "2.0"
project_hash = "733e56358cbd9948f7aa8b4fac162b08f8c2f57a"
...
git-tree-sha1 = "3107147167bffdf400025728d4708833c214a418"
...
[[deps.VoronoiFVM]]
deps = ["ADTypes", "BandedMatrices", "Colors", "CommonSolve", "DiffResults", "DifferentiationInterface", "DocStringExtensions", "ExtendableGrids", "ExtendableSparse", "ForwardDiff", "GridVisualize", "InteractiveUtils", "JLD2", "LinearAlgebra", "LinearSolve", "Printf", "Random", "RecursiveArrayTools", "RecursiveFactorization", "SciMLBase", "SciMLOperators", "SciMLPublic", "SparseArrays", "SparseConnectivityTracer", "SparseMatrixColorings", "StaticArrays", "Statistics", "TextWrap"]
git-tree-sha1 = "b654e0dfa47d48eecfce2bc7392f82d43612ba29"
uuid = "82b139dc-5afc-11e9-35da-9b9bdfd336f3"
version = "3.2.0"
weakdeps = ["ExtendableFEMBase", "PreallocationTools"]

    [deps.VoronoiFVM.extensions]
    VoronoiFVMExtendableFEMBaseExt = "ExtendableFEMBase"
    VoronoiFVMPreallocationToolsExt = "PreallocationTools"
